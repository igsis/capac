...
$pjObjeto =  new PessoaJuridicaController();
$oficinaObj = new OficinaController();

if (isset($_GET['idPj'])) {
    $_SESSION['pj_id_c'] = $idPj = $_GET['idPj'];
} elseif (isset($_SESSION['pj_id_c'])){
    $idPj = $_SESSION['pj_id_c'];
} else {
    $idPj = null;
}

if ($idPj) {
    $pj = $pjObjeto->recuperaPessoaJuridica($idPj);
    $_SESSION['pj_id_c'] = $idPj;
    $documento = $pj['cnpj'];
}

if (isset($_POST['pj_cnpj'])){
    $documento = $_POST['pj_cnpj'];
    $pj = $pjObjeto->getCNPJ($documento)->fetch();
    if ($pj){
        $idPj = (new MainModel)->encryption($pj['id']);
        $pj = $pjObjeto->recuperaPessoaJuridica($idPj);
        $_SESSION['pj_id_c'] = $idPj;
        $documento = $pj['cnpj'];
    }
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="card card-info">
                    ...
                    <!-- /.card-header -->
                    <!-- form start -->
                    <form class="form-horizontal formulario-ajax" method="POST" action="<?= SERVERURL ?>ajax/oficinaAjax.php" role="form" data-form="<?= ($idPj) ? "update" : "save" ?>">
                        <input type="hidden" name="_method" value="<?= ($idPj) ? "editarPj" : "cadastrarPj" ?>">
                        <input type="hidden" name="pj_ultima_atualizacao" value="<?= date('Y-m-d H-i-s') ?>">
                        <input type="hidden" name="pagina" value="<?= $_GET['views'] ?>">
                        <?php if ($idPj): ?>
                            <input type="hidden" name="id" value="<?= $idPj ?>">
                        <?php endif; ?>
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="form-group col-md-2">
                                    ...
                                    <input type="text" name="pj_cnpj" class="form-control" id="cnpj" value="<?= $documento ?>" readonly>
                                </div>
                                <div class="form-group col-md-2">
                                    <label for="ccm">CCM:</label>
                                    <input type="text" id="ccm" name="pj_ccm" class="form-control" placeholder="Digite o CCM" maxlength="11" value="<?= $pj['ccm'] ?? '' ?>">
                                </div>
                            </div>
                            <div class="row">
                            ...
                            <div class="row">
                                <div class="form-group col-md-4">
                                    ...
                                    <select required id="banco" name="bc_banco_id" class="form-control">
                                        ...
                                        <?php
                                        $pjObjeto->geraOpcao("bancos",$pj['banco_id'] ?? "");
                                        ?>
                                    </select>
                                </div>
                                <div class="form-group col-md-4">
